...
public class MaintenanceBookingController {
    ...
    @Operation(summary = "Cập nhật ghi chú lịch bảo trì — ADMIN, MANAGEMENT, SALE, TECHNICIAN")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Cập nhật thành công"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Không tìm thấy")
    })
    @PatchMapping("/{id}/notes")
    public ResponseEntity<ApiResponse<BookingResponse>> updateNotes(
            @PathVariable Long id,
            @RequestBody java.util.Map<String, String> body) {
        return ResponseEntity.ok(ApiResponse.success("Cập nhật ghi chú thành công",
                bookingService.updateNotes(id, body.getOrDefault("notes", ""))));
    }
